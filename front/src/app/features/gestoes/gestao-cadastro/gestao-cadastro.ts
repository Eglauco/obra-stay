...
import { GestaoRequest } from '../../../core/models/gestao.model';
...
/** Tela dedicada de criar/editar gestão (padrão de CRUD: sempre em nova tela). */
@Component({
...
export class GestaoCadastro {
  ...
  constructor() {
    afterNextRender(() => {
      ...
      else document.getElementById('ges-nome')?.focus();
    });
  }
  ...
  private carregar(id: number): void {
    ...
    this.service.obter(id).subscribe({
      next: (gestao) => {
        this.model.set({ nome: gestao.nome });
        this.carregando.set(false);
        queueMicrotask(() => document.getElementById('ges-nome')?.focus());
      },
      error: (e: HttpErrorResponse) => {
        this.carregando.set(false);
        this.erroCarregar.set(
          e.status === 404 ? 'Gestão não encontrada.' : this.mensagemErro(e),
        );
      },
    });
  }
  ...
  protected submit(): void {
    ...
    const req: GestaoRequest = { nome: this.model().nome.trim() };
    const id = this.id();
    ...
    op$.subscribe({
      next: () => {
        ...
        this.toast.success(
          id != null ? 'Gestão atualizada' : 'Gestão cadastrada',
  ...
  protected voltar(): void {
    ...
    else this.router.navigateByUrl('/gestoes');
  }
