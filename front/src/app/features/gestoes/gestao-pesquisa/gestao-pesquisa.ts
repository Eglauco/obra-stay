...
export class GestaoPesquisa {
  private readonly service = inject(GestaoService);
  ...
  protected readonly sortField = signal<GestaoSortField>('nome');
  protected readonly sortDir = signal<SortDir>('asc');

  // Resultado / estado
  protected readonly resultado = signal<PageResponse<Gestao> | null>(null);
  protected readonly loading = signal(false);
  protected readonly erro = signal<string | null>(null);

  // Exclusão
  protected readonly aExcluir = signal<Gestao | null>(null);
        ...
        switchMap(() => {
          ...
          return this.service.listar(this.filtroAtual()).pipe(
            catchError((e: HttpErrorResponse) => {
              ...
              return of<PageResponse<Gestao> | null>(null);
            }),
          );
        }),
        takeUntilDestroyed(),
  ...
  private filtroAtual(): GestaoFiltro {
    return {
      id: this.filtroId(),
  ...
  // ----- Ordenação -----
  protected ordenarPor(campo: GestaoSortField): void {
  ...
  protected ariaSort(campo: GestaoSortField): 'ascending' | 'descending' | 'none' {
  ...
  // ----- Exclusão -----
  protected pedirExclusao(g: Gestao): void {
    this.aExcluir.set(g);
  }
  ...
  protected confirmarExclusao(): void {
    ...
    this.service.excluir(g.id).subscribe({
      next: () => {
        ...
        this.toast.success('Gestão excluída', `${g.nome} foi removida.`);
        const r = this.resultado();
  ...
  protected mensagemExclusao(nome: string): string {
    return `Tem certeza que deseja excluir a gestão ${nome}? Esta ação não pode ser desfeita.`;
  }
