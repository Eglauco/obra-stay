...
export class FuncaoPesquisa {
  private readonly service = inject(FuncaoService);
  ...
  protected readonly sortField = signal<FuncaoSortField>('nome');
  protected readonly sortDir = signal<SortDir>('asc');

  // Resultado / estado
  protected readonly resultado = signal<PageResponse<Funcao> | null>(null);
  protected readonly loading = signal(false);
  protected readonly erro = signal<string | null>(null);

  // Exclusão
  protected readonly aExcluir = signal<Funcao | null>(null);
        ...
        switchMap(() => {
          ...
          return this.service.listar(this.filtroAtual()).pipe(
            catchError((e: HttpErrorResponse) => {
              ...
              return of<PageResponse<Funcao> | null>(null);
            }),
          );
        }),
        takeUntilDestroyed(),
  ...
  private filtroAtual(): FuncaoFiltro {
    return {
      id: this.filtroId(),
  ...
  // ----- Ordenação -----
  protected ordenarPor(campo: FuncaoSortField): void {
  ...
  protected ariaSort(campo: FuncaoSortField): 'ascending' | 'descending' | 'none' {
  ...
  // ----- Exclusão -----
  protected pedirExclusao(f: Funcao): void {
    this.aExcluir.set(f);
  }
  ...
  protected confirmarExclusao(): void {
    const f = this.aExcluir();
    if (!f || this.excluindo()) return;
    this.excluindo.set(true);
    this.service.excluir(f.id).subscribe({
      next: () => {
        this.excluindo.set(false);
        this.aExcluir.set(null);
        this.toast.success('Função excluída', `${f.nome} foi removida.`);
        const r = this.resultado();
  ...
  protected mensagemExclusao(nome: string): string {
    return `Tem certeza que deseja excluir a função ${nome}? Esta ação não pode ser desfeita.`;
  }
