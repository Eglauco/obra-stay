...
import { ApiError, FuncaoRequest } from '../../../core/models/colaborador.model';
...
/** Tela dedicada de criar/editar função (padrão de CRUD: sempre em nova tela). */
@Component({
  selector: 'app-funcao-cadastro',
  imports: [FormField],
  templateUrl: './funcao-cadastro.html',
  styleUrl: './funcao-cadastro.css',
})
export class FuncaoCadastro {
  ...
  constructor() {
    afterNextRender(() => {
      ...
      else document.getElementById('func-nome')?.focus();
    });
  }
  ...
  private carregar(id: number): void {
    ...
    this.service.obter(id).subscribe({
      next: (funcao) => {
        this.model.set({ nome: funcao.nome });
        this.carregando.set(false);
        queueMicrotask(() => document.getElementById('func-nome')?.focus());
      },
      error: (e: HttpErrorResponse) => {
        this.carregando.set(false);
        this.erroCarregar.set(
          e.status === 404 ? 'Função não encontrada.' : this.mensagemErro(e),
        );
      },
    });
  }
  ...
  protected submit(): void {
    ...
    const req: FuncaoRequest = { nome: this.model().nome.trim() };
    const id = this.id();
    ...
    op$.subscribe({
      next: () => {
        ...
        this.toast.success(
          id != null ? 'Função atualizada' : 'Função cadastrada',
  ...
  protected voltar(): void {
    ...
    else this.router.navigateByUrl('/funcoes');
  }
