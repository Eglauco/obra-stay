...
import { Local, LocalFiltro, LocalSortField } from '../../../core/models/local.model';

@Component({
  selector: 'app-local-pesquisa',
  imports: [ConfirmDialog, RouterLink],
  templateUrl: './local-pesquisa.html',
  styleUrl: './local-pesquisa.css',
})
export class LocalPesquisa {
  ...
  protected readonly filtroCidade = signal('');

  // Paginação / ordenação
  protected readonly page = signal(0);
  protected readonly size = signal(10);
  protected readonly sortField = signal<LocalSortField>('nome');
  ...
  // Exclusão
  protected readonly aExcluir = signal<Local | null>(null);
  ...
  private filtroAtual(): LocalFiltro {
    return {
      id: this.filtroId(),
      codigo: this.filtroCodigo().trim() || null,
      nome: this.filtroNome().trim() || null,
      cidade: this.filtroCidade().trim() || null,
      page: this.page(),
      size: this.size(),
  ...
  private hidratarDaUrl(): void {
    ...
    if (sort) {
      ...
      if (['id', 'codigo', 'nome', 'capacidade', 'cidade'].includes(campo)) {
        this.sortField.set(campo as LocalSortField);
      }
  ...
  protected onCidadeInput(valor: string): void {
    this.filtroCidade.set(valor);
    this.digitar$.next();
  }
  ...
  // ----- Ordenação -----
  protected ordenarPor(campo: LocalSortField): void {
  ...
  protected ariaSort(campo: LocalSortField): 'ascending' | 'descending' | 'none' {
  ...
  // ----- Exclusão -----
  protected pedirExclusao(l: Local): void {
    this.aExcluir.set(l);
  }
  ...
  protected confirmarExclusao(): void {
    const l = this.aExcluir();
    if (!l || this.excluindo()) return;
    this.excluindo.set(true);
    this.service.excluir(l.id).subscribe({
      next: () => {
        this.excluindo.set(false);
        this.aExcluir.set(null);
        this.toast.success('Local excluído', `${l.nome} foi removido.`);
        const r = this.resultado();
  ...
  protected mensagemExclusao(nome: string): string {
    return `Tem certeza que deseja excluir o local ${nome}? Esta ação não pode ser desfeita.`;
  }
