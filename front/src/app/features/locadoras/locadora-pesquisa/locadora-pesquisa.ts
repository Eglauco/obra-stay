...
export class LocadoraPesquisa {
  ...
  protected readonly sortField = signal<LocadoraSortField>('nome');
  protected readonly sortDir = signal<SortDir>('asc');

  // Resultado / estado
  protected readonly resultado = signal<PageResponse<Locadora> | null>(null);
  protected readonly loading = signal(false);
  protected readonly erro = signal<string | null>(null);

  // Exclusão
  protected readonly aExcluir = signal<Locadora | null>(null);
        ...
        switchMap(() => {
          ...
          return this.service.listar(this.filtroAtual()).pipe(
            catchError((e: HttpErrorResponse) => {
              ...
              return of<PageResponse<Locadora> | null>(null);
            }),
          );
        }),
        takeUntilDestroyed(),
  ...
  private filtroAtual(): LocadoraFiltro {
    return {
      id: this.filtroId(),
  ...
  // ----- Ordenação -----
  protected ordenarPor(campo: LocadoraSortField): void {
  ...
  protected ariaSort(campo: LocadoraSortField): 'ascending' | 'descending' | 'none' {
  ...
  // ----- Exclusão -----
  protected pedirExclusao(l: Locadora): void {
    this.aExcluir.set(l);
  }
  ...
  protected confirmarExclusao(): void {
    ...
    this.service.excluir(l.id).subscribe({
      next: () => {
        ...
        this.toast.success('Locadora excluída', `${l.nome} foi removida.`);
        const r = this.resultado();
  ...
  protected mensagemExclusao(nome: string): string {
    return `Tem certeza que deseja excluir a locadora ${nome}? Esta ação não pode ser desfeita.`;
  }
