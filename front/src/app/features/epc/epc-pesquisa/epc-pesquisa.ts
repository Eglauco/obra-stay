...
export class EpcPesquisa {
  ...
  protected readonly sortField = signal<EpcSortField>('nome');
  protected readonly sortDir = signal<SortDir>('asc');

  // Resultado / estado
  protected readonly resultado = signal<PageResponse<Epc> | null>(null);
  protected readonly loading = signal(false);
  protected readonly erro = signal<string | null>(null);

  // Exclusão
  protected readonly aExcluir = signal<Epc | null>(null);
        ...
        switchMap(() => {
          ...
          return this.service.listar(this.filtroAtual()).pipe(
            catchError((e: HttpErrorResponse) => {
              ...
              return of<PageResponse<Epc> | null>(null);
            }),
          );
        }),
        takeUntilDestroyed(),
  ...
  // ----- Ordenação -----
  protected ordenarPor(campo: EpcSortField): void {
  ...
  protected ariaSort(campo: EpcSortField): 'ascending' | 'descending' | 'none' {
  ...
  // ----- Exclusão -----
  protected pedirExclusao(e: Epc): void {
    this.aExcluir.set(e);
  }
  ...
  protected confirmarExclusao(): void {
    ...
    this.service.excluir(e.id).subscribe({
      next: () => {
        ...
        this.toast.success('EPC excluído', `${e.nome} foi removido.`);
        const r = this.resultado();
  ...
  protected mensagemExclusao(nome: string): string {
    return `Tem certeza que deseja excluir o EPC ${nome}? Esta ação não pode ser desfeita.`;
  }
