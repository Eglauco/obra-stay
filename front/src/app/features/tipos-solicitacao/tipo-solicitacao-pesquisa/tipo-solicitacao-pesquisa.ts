...
export class TipoSolicitacaoPesquisa {
  ...
  protected readonly sortField = signal<TipoSolicitacaoSortField>('nome');
  protected readonly sortDir = signal<SortDir>('asc');

  // Resultado / estado
  protected readonly resultado = signal<PageResponse<TipoSolicitacao> | null>(null);
  protected readonly loading = signal(false);
  protected readonly erro = signal<string | null>(null);

  // Exclusão
  protected readonly aExcluir = signal<TipoSolicitacao | null>(null);
        ...
        switchMap(() => {
          ...
          return this.service.listar(this.filtroAtual()).pipe(
            catchError((e: HttpErrorResponse) => {
              ...
              return of<PageResponse<TipoSolicitacao> | null>(null);
            }),
          );
        }),
        takeUntilDestroyed(),
  ...
  private filtroAtual(): TipoSolicitacaoFiltro {
    return {
      id: this.filtroId(),
  ...
  // ----- Ordenação -----
  protected ordenarPor(campo: TipoSolicitacaoSortField): void {
  ...
  protected ariaSort(campo: TipoSolicitacaoSortField): 'ascending' | 'descending' | 'none' {
  ...
  // ----- Exclusão -----
  protected pedirExclusao(l: TipoSolicitacao): void {
    this.aExcluir.set(l);
  }
  ...
  protected confirmarExclusao(): void {
    ...
    this.service.excluir(l.id).subscribe({
      next: () => {
        ...
        this.toast.success('Tipo de solicitação excluído', `${l.nome} foi removido.`);
        const r = this.resultado();
  ...
  protected mensagemExclusao(nome: string): string {
    return `Tem certeza que deseja excluir o tipo de solicitação ${nome}? Esta ação não pode ser desfeita.`;
  }
